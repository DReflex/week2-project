document.onreadystatechange = function () {
    if (document.readyState == "interactive") {
        ...
        //elements
        var button = document.getElementsByClassName('toggle-popup');
        ...
        for (var i = 0; i < button.length; i++) {
          button[i].addEventListener('click', toggle)
        }
        function toggle(){
          var randomPost= getRandomInt(0, posts.length);
          popup.style.display="block";
          popupBody.style.background="rgba(0, 0, 0, 0.75)";
          popupBody.style.height= document.body.clientHeight + "px";

          title.innerHTML= posts[randomPost].title;
          image.src=posts[randomPost].img;
          date.innerHTML = posts[randomPost].date;
          text.innerHTML = posts[randomPost].text;
          link.href = posts[randomPost].link
